...
这个干净版本适合直接推到 GitHub：仓库里不需要提交 `node_modules/`、`dist/`、本地考试库、考试记录、API Key、音频或图片资源。

推到 GitHub 后，`.github/workflows/build.yml` 会在 GitHub Actions 中自动构建：

- macOS：`IELTS Mock Lab-*-mac.zip`
- Windows x64：`IELTS Mock Lab-*-win.zip`

构建产物可以在 Actions 页面的 Artifacts 下载；如果要正式分发，建议把这些 zip 上传到 GitHub Releases。
