...
const state = {
  ...
  libraryCollapsed: localStorage.getItem(LIBRARY_COLLAPSED_STORAGE_KEY) === "true",
  historyCollapsed: localStorage.getItem(HISTORY_COLLAPSED_STORAGE_KEY) === "true",
  assets: new Map(),
  answers: {},
  review: new Set(),
...
async function handleSpeakingBankInput(event) {
  ...
  if (!file) return;
  try {
    const bank = normalizeSpeakingBank(JSON.parse(await file.text()));
...
async function loadSpeakingBank() {
  try {
    const saved = JSON.parse(localStorage.getItem(SPEAKING_BANK_STORAGE_KEY) || "null");
...
function renderAiSettings() {
  ...
  document.querySelectorAll("[data-ai-feature][data-ai-field='providerKey']").forEach((select) => {
    select.addEventListener("change", () => applyProviderPreset(select));
  });
}
...
function saveAiSettingsFromForm() {
  ...
  state.aiSettings = mergeAiSettings(next);
  localStorage.setItem(AI_SETTINGS_STORAGE_KEY, JSON.stringify(state.aiSettings));
  els.aiSettingsStatus.textContent = "已保存到本地";
}
...
function resetAiSettings() {
  ...
  localStorage.setItem(AI_SETTINGS_STORAGE_KEY, JSON.stringify(state.aiSettings));
  renderAiSettings();
  els.aiSettingsStatus.textContent = "已恢复默认";
}
...
async function generateExamWithAi() {
  ...
  try {
    ...
    stage = "逐页 OCR / 页面转写";
    setAiStatus("正在逐页解析 PDF/图片...");
    const test = await callAiForExam(material, (text) => {
      stage = text;
      setAiStatus(text);
    });
    attachListeningTranscriptContext(test, material);
    stage = "处理手动 PDF 页截图";
    setAiStatus("正在处理手动指定的 PDF 页截图...");
    ...
    setAiStatus(`已生成并加入考试库：${test.title || "Untitled Test"}${warningText}`);
  } catch (error) {
    console.error(error);
...
function loadImage(dataUrl) {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error("无法加载待裁剪图片。"));
    image.src = dataUrl;
  });
}
...
async function extractPagesWithOcr(material, onStatus = () => {}) {
  ...
  onStatus("未启用 GLM-OCR，跳过 PDF/图片 OCR。");
  return [];
}
...
async function transcribePagesWithGlmOcr(inputs, material, onStatus = () => {}) {
  ...
  for (let index = 0; index < inputs.length; index += 1) {
    ...
    onStatus(`OCR：正在解析第 ${index + 1}/${inputs.length} 个文件：${input.label || input.name}...`);
    try {
  ...
  onStatus(`OCR：已返回 ${pages.filter(Boolean).length}/${inputs.length} 个文件`);
  return pages;
}
...
async function mergeExtractedTextToExam(material, extractedPages, onStatus = () => {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    onStatus(attempt === 1 ? "正在用纯文本上下文合并完整 JSON..." : "正在让 AI 根据校验错误修正 JSON...");
    const content = await requestTextCompletionWithFallback(messages, "merge");
    onStatus("AI 已返回内容，正在解析和校验完整性...");
    try {
...
async function handleJsonInput(event) {
  ...
  try {
    state.pendingTest = parseImportedTests(JSON.parse(await file.text()));
...
function saveLibrary() {
  ...
  try {
    localStorage.setItem(LIBRARY_STORAGE_KEY, JSON.stringify(serializable.slice(0, 50)));
  } catch (error) {
    ...
    try {
      localStorage.setItem(LIBRARY_STORAGE_KEY, JSON.stringify(withoutGeneratedAssets.slice(0, 50)));
    } catch (fallbackError) {
      console.warn("Library could not be saved.", fallbackError);
      alert("考试已加入当前页面，但本地考试库保存失败。可能是浏览器本地存储空间已满。");
    }
  }
}
...
function serializeGeneratedAssets(assets) {
  ...
  (assets || new Map()).forEach((asset, key) => {
    ...
    seen.add(canonical);
    serialized.push({ name, dataUrl: asset.dataUrl });
  });
  return serialized;
}
  ...
  els.partTabs.innerHTML = state.test.sections.map((section, index) => {
    ...
    const progress = state.test.testType === "writing"
      ...
      : state.test.testType === "speaking"
        ...
        : `${answered}/${total}`;
    return `
      <button class="part-tab ${index === state.currentSectionIndex ? "active" : ""}" data-section="${index}" type="button">
...
function renderInlineInput(question, group) {
  ...
  return `
    <span class="inline-answer ${state.currentQuestionId === id ? "current" : ""} ${isAnswered(id) ? "answered" : ""} ${review ? "review" : ""}" data-question-id="${escapeAttribute(id)}">
      ...
      <button class="review-toggle inline-review ${review ? "active" : ""}" type="button" title="标记复查" data-review="${escapeAttribute(id)}">!</button>
    </span>
  `;
}
...
function renderQuestion(question, group) {
  ...
  return `
    <div class="question ${state.currentQuestionId === id ? "current" : ""} ${answered ? "answered" : ""} ${review ? "review" : ""}" data-question-id="${escapeAttribute(id)}">
      <button class="review-toggle ${review ? "active" : ""}" type="button" title="标记复查" data-review="${escapeAttribute(id)}">!</button>
...
function renderAnswerControl(question, group, value) {
  ...
  if (group.questionType === "blank") {
    return `<input class="answer-input" data-answer="${escapeAttribute(id)}" type="text" value="${escapeAttribute(value || "")}" autocomplete="off" />`;
  }
  const inputType = group.questionType === "multi_choice" ? "checkbox" : "radio";
  const selected = Array.isArray(value) ? value : value ? [value] : [];
  return `
    <div class="option-list" data-choice-group="${escapeAttribute(id)}" data-max="${Number(group.maxChoices || question.maxChoices || 99)}">
      ${(question.options || []).map((option) => {
        ...
        return `
          <label class="option">
            <input type="${inputType}" name="q_${escapeAttribute(id)}" data-answer="${escapeAttribute(id)}" value="${escapeAttribute(optionValue)}" ${checked} />
  ...
  els.questionNav.innerHTML = state.questionIndex.flatMap((item) => {
    return expandQuestionLabels(item).map((label) => `
      <button class="nav-number ${state.currentQuestionId === item.id ? "current" : ""} ${isAnswered(item.id) ? "answered" : ""} ${state.review.has(item.id) ? "review" : ""}"
        data-nav="${escapeAttribute(item.id)}" type="button">${escapeHtml(label)}</button>
    `);
  }).join("");
...
function setupAudioGuards() {
  els.audioStartBtn.addEventListener("click", async () => {
    if (!els.audioPlayer.src) {
      alert("当前听力没有找到音频。请导入 JSON 中引用的音频文件。");
      return;
    }
    try {
...
function saveHistory(result) {
  ...
  try {
    ...
    alert("本次结果已生成，但历史记录保存失败。可能是本地存储空间已满。");
  }
  renderHistoryList();
}
...
function updateSavedHistoryItem(result) {
  ...
  try {
    localStorage.setItem("ielts-mock-history", JSON.stringify(history));
  } catch (error) {
...
function safeLoadHistory() {
  try {
    const history = JSON.parse(localStorage.getItem("ielts-mock-history") || "[]");
